...
namespace SmartHealthPredication
{
    public partial class PatientLogin : System.Web.UI.Page
    {
        ...
        protected void btnPatientLogin_Click(object sender, EventArgs e)
        {
            using (var db = new SmartHealthPredicationEntities())
            {
                ...
                if (login != null)
                {
                    ...
                    Response.Redirect("DiseasePredication");
                }
                else
                {
        ...
        protected void btnDocotrLoing_Click(object sender, EventArgs e)
        {
            using (var db = new SmartHealthPredicationEntities())
            {
                tblUserRegistration login = db.tblUserRegistration.Where(a => a.EmailID == PatientLoginEmailID.Text && a.Password == PatientloginPassword.Text && a.RoleID == 2 && a.Status == "Active").FirstOrDefault();
                if (login != null)
                {
